...
pub struct MonteCarloNode<G: MonteCarloGameData, A: MonteCarloPlayerAction, U: MonteCarloGameDataUpdate> {
    game_data: G,
    player_action: A,
    game_data_update: U,
...
impl <G: MonteCarloGameData, A: MonteCarloPlayerAction, U: MonteCarloGameDataUpdate> MonteCarloNode<G, A, U> {
    ...
    fn apply_action(&mut self, parent_game_data: &G, parent_action: &A, game_mode: MonteCarloGameMode, max_number_of_turns: usize, use_heuristic_score: bool) -> bool {
        ...
        match game_mode {
            MonteCarloGameMode::SameTurnParallel => {
                if self.player == MonteCarloPlayer::Me {
                    ...
                    self.game_data.simultaneous_player_actions_for_simultaneous_game_data_change(parent_action, &self.player_action);
                }
            },
...
pub struct MonteCarloTreeSearch<G: MonteCarloGameData, A: MonteCarloPlayerAction, U: MonteCarloGameDataUpdate> {
...
impl <G: MonteCarloGameData, A: MonteCarloPlayerAction, U: MonteCarloGameDataUpdate> MonteCarloTreeSearch<G, A, U> {
    pub fn new(game_mode: MonteCarloGameMode, max_number_of_turns: usize, force_update: bool, time_out_first_turn: Duration, time_out_successive_turns: Duration, weighting_factor: f32, use_heuristic_score: bool, debug: bool, keep_root: bool) -> Self {
    ...
    pub fn choose_and_execute_actions(&mut self) -> (impl MonteCarloGameData, impl MonteCarloPlayerAction) {
                        ...
                        match node_type {
                            MonteCarloNodeType::ActionResult => {
                                ...
                                let apply_player_actions_to_game_data = match self.game_mode {
                                    MonteCarloGameMode::SameTurnParallel => child.get_value().player == MonteCarloPlayer::Me,
        ...
        for i in 0..50 {
            ...
            while !ttt_match.check_game_ending(0) {
                ...
                for child in parent.iter_children() {
                    ...
                    let child_action = TicTacToePlayerAction::downcast_self(&child_node.player_action);
