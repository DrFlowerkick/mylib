    ...
    SP: SimulationPolicy<G, H>,
{
    fn set_root(&mut self, state: &G::State) -> anyhow::Result<bool> {
        ...
        if let Some(root_id) = self.tree.root_id() {
            ...
            if let Some((new_root_id, _)) = self
                ...
                .get_children()
                .iter()
                .filter_map(|&my_move_node_id| self.tree.get_node(my_move_node_id).ok())
                .flat_map(|my_move_node| {
                    my_move_node
                        .get_children()
                        .iter()
                        .filter_map(|&opponent_move_node_id| {
                            self.tree.get_node(opponent_move_node_id).ok().map(
                                |opponent_move_node| {
                                    (opponent_move_node_id, opponent_move_node.get_state())
                                },
                            )
                        })
                })
    ...
    fn iterate(&mut self) -> anyhow::Result<()> {
        ...
        let root_id = tree
            .root_id()
            .context("Root node must be initialized before iterating")?;
        ...
        while !tree.get_node(current_id)?.get_children().is_empty() {
            ...
            for vec_index in 0..num_parent_children {
                let child_index = tree
                    .get_node(current_id)
                    .context("Current node does not exist")?
                    .get_children()[vec_index];
                let utc = tree
                    .get_node_mut(child_index)
                    .context("Child node does not exist")?
            ...
            let Some(best_child_index) = best_child_index else { return Err(anyhow::anyhow!("Could not find best child index."))};
    ...
    fn select_move(&self) -> anyhow::Result<&G::Move> {
        let root_id = self
            .tree
            .root_id()
            .context("Root node must be initialized before selecting a move")?;
        let (move_id, _) = self
            .tree
            .get_node(root_id)?
            .get_children()
            .iter()
            .filter_map(|&child_id| self.tree.get_node(child_id).ok().map(|n| (child_id, n)))
            .max_by_key(|(_, child_node)| child_node.get_visits())
            .context("could not find move id")?;
        self.tree
            .get_node(move_id)?
            .get_move()
            .context("node did not contain move")
    }
}
